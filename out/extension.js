    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        if (registerServerEventMatch) {
            ...
            if (!nextLine.includes('AddEventHandler')) {
                ...
                const diagnostic = new vscode.Diagnostic(range, 'Consider using RegisterNetEvent("event", function(...) end) instead of separate RegisterServerEvent and AddEventHandler.', vscode.DiagnosticSeverity.Information);
        ...
        if (sourceCommaMatch) {
            ...
            const range = new vscode.Range(new vscode.Position(lineIndex, matchIndex), new vscode.Position(lineIndex, matchIndex + sourceCommaMatch[0].length));
            const diagnostic = new vscode.Diagnostic(range, 'Consider adding space after comma in function parameters: function(source, ...).', vscode.DiagnosticSeverity.Hint);
...
function checkCitizenPatterns(document) {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        if (line.includes('Citizen.CreateThread')) {
            const range = new vscode.Range(new vscode.Position(lineIndex, line.indexOf('Citizen.CreateThread')), new vscode.Position(lineIndex, line.indexOf('Citizen.CreateThread') + 'Citizen.CreateThread'.length));
            const diagnostic = new vscode.Diagnostic(range, 'Use CreateThread instead of Citizen.CreateThread.', vscode.DiagnosticSeverity.Information);
